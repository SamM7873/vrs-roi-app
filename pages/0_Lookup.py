...
CONVO_NOW_RATE_PER_MINUTE = 2.80
...
def render_table_and_summary(df):
    ...
    net = cost_diff.sum()
    net_color = "#2FB344" if net >= 0 else "#EF4444"
  ...
  {stat_tile("Profit / Loss Months", f"{profit_count}P · {loss_count}L", f"of {total_months} total months")}
    ...
    header = """<div style="display:grid;grid-template-columns:130px 1fr 1fr 1fr 1fr 1fr 110px;
                gap:0.75rem;padding:0.7rem 1.25rem;background:#F1F3F8;border-radius:10px 10px 0 0;
...
def render_profit_loss_summary(df):
    ...
    def _mini_tiles(*items):
        tiles = "".join(
            f"""<div style="background:#F1F3F8;border:1px solid #2d6b47;border-radius:12px;padding:0.9rem 1.1rem;">
  ...
  <div style="background:#F1F3F8;border:1px solid #2d6b47;border-radius:12px;padding:0.9rem 1.1rem;">
    ...
    <div style="font-size:1.3rem;font-weight:800;color:#2FB344;">{len(cost_profit_months)}</div>
...
.stTabs [data-baseweb="tab"][aria-selected="true"] { background: #F1F3F8 !important; color: #3d3430 !important; border-bottom: 2px solid #2FB344 !important; }
...
.stSelectbox [data-baseweb="select"] > div { background: #F1F3F8 !important; border-color: #2d6b47 !important; color: #3d3430 !important; }
...
with mid:
    ...
    st.markdown("""
<div style="background:linear-gradient(135deg,#0D3B26 0%,#12492F 100%);
  ...
  <svg width="120" height="24" viewBox="-80 0 1600 315" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path fill-rule="evenodd" clip-rule="evenodd" d="M86.1444 48.6372C117.108 48.6372 147.181 61.4449 168.357 83.5988C177.077 92.5988 190.779 94.6757 201.1 88.2719C207.328 84.4642 211.421 78.2334 212.311 71.1372C213.2 64.0411 211.065 56.9449 206.082 51.7526C174.941 18.3488 130.454 -0.689685 84.187 0.0026227C40.9453 0.521854 0.194893 18.0026 -30.4124 49.1565C-61.0197 80.3103 -77.2131 120.637 -75.7895 162.695C-74.3659 203.541 -56.9269 241.791 -26.4975 270.522C3.93183 299.253 43.9704 315.003 86.1444 315.003C128.318 315.003 175.119 296.31 205.726 263.599C210.353 258.58 212.666 252.176 212.311 245.426C211.777 238.849 208.752 232.791 203.413 228.464C193.092 220.156 177.966 221.022 168.891 230.714C145.579 255.637 112.481 268.445 77.9588 266.022C22.9724 262.214 -21.6929 218.945 -25.6078 165.464C-27.9211 134.83 -17.2441 105.753 4.10978 83.2526C25.2858 61.0988 55.1812 48.4642 86.1444 48.4642V48.6372Z" fill="white"/>
    <path fill-rule="evenodd" clip-rule="evenodd" d="M514.477 157.5C514.477 217.558 464.295 266.365 402.369 266.365C340.443 266.365 290.261 217.558 290.261 157.5C290.261 97.4423 340.443 48.6346 402.369 48.6346C464.295 48.6346 514.477 97.4423 514.477 157.5ZM402.547 0C313.216 0 240.435 70.6154 240.435 157.5C240.435 244.385 313.038 315 402.369 315C491.7 315 564.303 244.385 564.303 157.5C564.303 70.6154 491.877 0 402.547 0Z" fill="white"/>
    <path fill-rule="evenodd" clip-rule="evenodd" d="M1465.79 157.5C1465.79 217.558 1415.61 266.365 1353.68 266.365C1291.76 266.365 1241.57 217.558 1241.57 157.5C1241.57 97.4423 1291.76 48.6346 1353.68 48.6346C1415.61 48.6346 1465.79 97.4423 1465.79 157.5ZM1353.86 0C1264.53 0 1191.93 70.6154 1191.93 157.5C1191.93 244.385 1264.53 315 1353.86 315C1443.19 315 1515.79 244.385 1515.79 157.5C1515.79 70.6154 1443.19 0 1353.86 0Z" fill="white"/>
    <path fill-rule="evenodd" clip-rule="evenodd" d="M1167 1.90617C1154.37 -3.28614 1139.6 2.59848 1134.26 14.8869L1042.08 227.945L949.907 14.8869C944.568 2.59848 929.976 -3.28614 917.164 1.90617C904.53 7.09848 898.479 21.2908 903.818 33.7523L1019.13 300.118C1023.04 309.118 1032.12 315.002 1042.08 315.002C1052.05 315.002 1061.12 309.118 1065.04 300.118L1180.35 33.7523C1183.02 27.6946 1183.02 21.1177 1180.35 15.2331C1177.86 9.1754 1173.06 4.50233 1166.83 2.07925L1167 1.90617Z" fill="white"/>
  </svg>
...
if "search_results" in st.session_state:
    ...
    saved_color = "#2FB344" if cost_saved >= 0 else "#EF4444"
  ...
  {_tile("Total VRS Min", f"{total_vrs_min:,.1f}", icon="⏱️", color="#0EA5E9", chip="#E4F5FC")}
    ...
    # Group numbers by person (email) — VRS records first
    from collections import defaultdict as _pdd
    _person_groups = _pdd(list)
    for _r in sorted(matched_numbers, key=lambda r: 0 if norm(r.get("properties", {}).get("service_type") or "") == "vrs" else 1):
    ...
    def _render_person_detail(person_records):
        ...
        for r in person_records:
            ...
            if _sec_lbl != _last_lbl and len(person_records) > 1:
                _sec_color = "#2FB344" if svc == "vrs" else "#3B82F6"
            ...
            if is_vrs:
                ...
                diff_color = "#2FB344" if diff_m >= 0 else "#EF4444"
                diff_sign = "+" if diff_m >= 0 else ""
                monthly_col_html = (
                    '<div style="background:#FFFFFF;border:1px solid #E6E9F0;border-radius:14px;padding:1.15rem;height:100%;box-shadow:0 1px 2px rgba(24,36,51,.04),0 4px 14px rgba(24,36,51,.06);">'
                    + card_header("Monthly Value (VRS)")
                    + f'<div style="font-size:1.3rem;font-weight:800;color:#2FB344;margin-bottom:0.3rem;">{current_m:,.1f} min</div>'
            ...
            if is_vrs:
                ...
                if primary_addr.strip():
                    ...
                    if coords:
                        map_points.append({"lat": coords[0], "lon": coords[1], "label": "Primary Address", "addr": primary_addr, "color": "#2FB344"})
...
<div style="display:flex;flex-wrap:wrap;gap:0.75rem;margin-bottom:0.6rem;">
  {legend_items}
                    ...
                    fig = px.scatter_mapbox(
                        ...
                        color="label",
                        color_discrete_map={"Primary Address": "#2FB344", "Emergency Address": "#EF4444"},
                        zoom=11, height=260,
                    )
        ...
        if _p_email:
            ...
            if _pendo and (_pendo.get("convo_now_account_id") or "").strip():
                ...
                try:
                    ...
                    _trend_html = (f'<span style="color:{"#2FB344" if _trend_f >= 0 else "#EF4444"};font-weight:700;">'
    ...
    if _profile_key:
        ...
        _pfp_svc_color = "#2FB344" if _pfp_svc == "vrs" else "#3B82F6"
        _pfp_svc_tag = "VRS" if _pfp_svc == "vrs" else "Convo Now"
        _pfp_status = (_pfp.get("number_status") or "").title()
        _pfp_stat_color = "#2FB344" if _pfp_status.lower() == "live" else "#EF4444" if _pfp_status.lower() == "suspended" else "#6B7280"
...
""", unsafe_allow_html=True)
        ...
        for _person_key, _person_records in _person_groups.items():
            ...
            _svc_color = "#2FB344" if _list_svc == "vrs" else "#3B82F6"
            _stat_color = "#2FB344" if _list_status.lower() == "live" else "#EF4444" if _list_status.lower() == "suspended" else "#6B7280"
...
<div style="background:#FFFFFF;border:1px solid #E6E9F0;
  ...
  <div style="width:38px;height:38px;border-radius:50%;
      background:{_svc_color}30;border:1.5px solid {_svc_color}70;
      display:flex;align-items:center;justify-content:center;
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.5rem;">
  <div style="background:linear-gradient(135deg,#2FB344,#B59467);border-radius:14px;padding:1.25rem 1.5rem;color:#fff;">
