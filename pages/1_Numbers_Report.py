...
if report_df is None or report_df.empty:
    ...
    if not df_dated.empty:
        ...
        def bar_chart(data, x_col, x_title, sort_order):
            if data.empty:
                ...
                return
            chart = alt.Chart(data).mark_bar(color="#C9A876", cornerRadiusTopLeft=4, cornerRadiusTopRight=4).encode(
        ...
        if not monthly_status.empty:
            chart = alt.Chart(monthly_status).mark_bar().encode(
                ...
                y=alt.Y("Count:Q", title="Numbers Created"),
                color=alt.Color("Status:N", scale=alt.Scale(domain=["Active", "Live"], range=["#C9A876", "#FFA500"]), legend=alt.Legend(title="Status")),
    ...
    # Refresh button (optional - clears cache and reloads)
