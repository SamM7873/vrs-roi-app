...
# minutes generated by the reactivated group (VRS minutes in the measure month)
react_df = df[oc == O_REACT]
react_min = int(round(react_df["VRS Min (measure)"].sum())) if "VRS Min (measure)" in df.columns else 0
...
_card(h[1], "⏱️ Minutes by reactivated", f"{react_min:,}",
      f"VRS minutes in {saved.get('measure','')} · avg {react_avg:,}/account", "#0FB5AE")
st.markdown("")
