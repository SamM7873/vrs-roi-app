...
if run and up is not None:
    ...
    for pr in people:
        ...
        # cumulative VRS after baseline (all months after base_to) — "so far"
        since_vrs = round(sum(v for (nn, m), v in d_vrs.items()
                              if nn in vrs_valid and m > base_to), 1)
        ...
        rows.append({
            ...
            "VRS Min (since campaign)": since_vrs,
...
# minutes generated by the reactivated group
react_df = df[oc == O_REACT]
_sincecol = "VRS Min (since campaign)" if "VRS Min (since campaign)" in df.columns else "VRS Min (measure)"
react_min = int(round(react_df[_sincecol].sum())) if _sincecol in df.columns else 0
react_avg = round(react_min / n_react, 1) if n_react else 0

# ── THE TWO KEY NUMBERS (for the campaign readout) ────────────────────────────────
h = st.columns(2)
_card(h[0], "🚀 Reactivated accounts", f"{n_react:,}",
      f"{_pct(n_react)} of audience · was 0 → generating now", "#2DB84B")
_card(h[1], "⏱️ VRS minutes by reactivated (so far)", f"{react_min:,}",
      f"generated by the {n_react:,} reactivated · avg {react_avg:,}/account", "#0FB5AE")
st.markdown("")
