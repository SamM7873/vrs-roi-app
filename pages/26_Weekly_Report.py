...
st.set_page_config(page_title="Weekly Report", layout="wide", page_icon="🗓️")
st.markdown(COMMON_CSS, unsafe_allow_html=True)
require_auth()
log_report_view("Weekly Report")
...
def _load():
    ...
    m0 = _today - timedelta(days=_today.weekday())   # Monday of this week
    m1 = m0 + timedelta(days=7)                        # next Monday
    pm0 = m0 - timedelta(days=7)                       # Monday of last week
    ...
    return {
        ...
        # created this week but with NO registration timestamp (unregistered)
...
with h1:
    st.markdown(
        ...
        "<div style='font-size:1.7rem;font-weight:800;color:#1A2234;letter-spacing:-0.5px;margin-top:-2px;'>Weekly Report</div>",
        unsafe_allow_html=True)
with h2:
    st.markdown("<div style='height:0.9rem;'></div>", unsafe_allow_html=True)
    refresh = st.button("↻ Refresh", use_container_width=True)

_c = st.session_state.get("_wr")
if refresh or (not _c) or (time.time() - _c.get("ts", 0)) > TTL:
    with st.spinner("Loading this week's metrics…"):
        _c = _load()
        st.session_state["_wr"] = _c
...
with k1:
    _kpi("New VRS numbers", vrs_m, f"vs {vrs_p:,} last week", BLUE, _delta(vrs_m, vrs_p))
    st.altair_chart(_spark(BLUE, "bar", "VRS"), use_container_width=True)
with k2:
    _kpi("New Convo Now", cn_m, f"vs {cn_p:,} last week", GREEN, _delta(cn_m, cn_p))
    st.altair_chart(_spark(GREEN, "line", "ConvoNow"), use_container_width=True)
with k3:
    _kpi("Total new numbers", tot_m, f"vs {tot_p:,} last week", CYAN, _delta(tot_m, tot_p))
    st.altair_chart(_spark(CYAN, "area", "Total"), use_container_width=True)
with k4:
    _kpi("New registrations", reg, f"vs {reg_p:,} last week", PURPLE, _delta(reg, reg_p))
    st.markdown(f"""<div style="height:6px;background:#EEF1F6;border-radius:4px;margin-top:0.55rem;overflow:hidden;">
      <div style="width:{min(reg/(reg_p or 1)*50,100):.0f}%;height:100%;background:{PURPLE};"></div></div>""",
                unsafe_allow_html=True)
with k5:
    _kpi("Net new", _net, f"vs {_net_prev:,} last week {_delta(_net, _net_prev)}",
...
if _ur_rows:
    st.markdown("<div style='height:1.2rem;'></div>", unsafe_allow_html=True)
    st.markdown(f"##### Unregistered numbers this week · {len(_ur_rows):,}")
    st.caption("Created this week with no registration timestamp — typically added manually by "
