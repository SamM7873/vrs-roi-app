...
if not _waited.empty:
    with st.expander(f"⏳ Who waited longest (LWT = {_ms_lbl(_lwt_sec)}) — all {len(_waited):,}",
        ...
        _wcols = ["Day", "Customer", "Agent", "Source", "Wait", "Status"]
        st.caption(f"All {len(_top):,} interactions with a wait, longest first. The top row is the **LWT**.")
        st.dataframe(_top[_wcols], use_container_width=True, hide_index=True, height=460)
        st.download_button("📥 Export waits (CSV)", _top[_wcols].to_csv(index=False),
                           "who_waited_longest.csv", "text/csv", key="ivt_wait_csv")
        st.caption("📵 Missed = the caller hung up / was dropped before connecting; "
                   "✅ Connected = they waited this long, then were answered.")
