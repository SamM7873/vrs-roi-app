...
if "last_refresh_geo" not in st.session_state:
    st.session_state.last_refresh_geo = time.time()

current_time = time.time()
if current_time - st.session_state.last_refresh_geo > 600:  # 10 minutes
    st.session_state.last_refresh_geo = current_time
    st.rerun()

@persistent_cache(ttl_seconds=600)  # 10 minutes
def fetch_geo_data():
    """Fetch geographic data and cache for 10 minutes (persists across sessions)"""
...
if geo_df is None or geo_df.empty:
    ...
    with tab_primary:
        ...
        fig_state = px.choropleth(
            ...
            color_continuous_scale=[[0, "#F5F3F0"], [0.5, "#C9A876"], [1, "#B59467"]],
        ...
        fig_city = go.Figure(go.Bar(
            ...
            y=top_cities["Count"], marker_color="#C9A876", marker_line_width=0,
