...
def _load():
    ...
    m0 = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)   # this month start
    m1 = m0 + relativedelta(months=1)                                     # next month start
    pm0 = m0 - relativedelta(months=1)                                    # prev month start
...
r2 = st.columns(3)
card(r2[0], "Deactivated this month", _c["deact"], "closed / deleted", RED, "🚫", "#FDECEC")
card(r2[1], "Port-Out this month", _c["portout"], "ported to another provider", AMBER, "📤", "#FEF3E2")
_net = _v(_c["new_vrs"]) + _v(_c["new_convo"]) - _v(_c["deact"])
card(r2[2], "Net new (VRS+Convo − deact)", _net, "growth this month", GREEN if _net >= 0 else RED, "📈", "#E7F6EC")
...
st.caption("This dashboard is scoped to the current calendar month. Use **Overview** for all-time totals.")
