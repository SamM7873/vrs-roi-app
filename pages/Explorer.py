...
report_header("Data Explorer", "Browse and filter all custom object properties")
...
object_type = st.sidebar.selectbox("Select Object", ["VRS Numbers", "Registrations", "Monthly Values"])

@persistent_cache(ttl_seconds=600)
def fetch_data(obj_type):
    if obj_type == "VRS Numbers":
        return list_all("2-40974683", 
            ["number", "email", "first_name", "last_name", "number_status", "service_type", "usage_type", "number_created_at"],
            progress_label="Fetching VRS Numbers")
    elif obj_type == "Registrations":
        return list_all("2-58833629",
            ["registration_id", "registration_type", "email", "first_name", "last_name", "number", "submitted_at"],
...
if not records:
    ...
    st.download_button("📥 Download CSV", csv, f"{object_type.lower().replace(' ', '_')}.csv", "text/csv")
