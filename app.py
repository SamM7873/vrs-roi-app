...
if st.button("Search") and search_input.strip():
    ...
    if not matched_numbers:
        ...
        # --- Group number-object records into "persons" by normalized (lowercased) email ---
        num_to_person = {}
        ...
        st.write(f"Distinct numbers found: {distinct_numbers}")
        ...
        # --- Collect per-person, per-month VRS/CFZ/Convo Now values (lists, to be summed) ---
        ...
        # --- Build merged rows: one per person per month ---
        rows = []
        ...
        # --- Summary with PROFIT/LOSS percentages ---
        ...
        # --- Chart per merged person, with custom colors via Altair ---
